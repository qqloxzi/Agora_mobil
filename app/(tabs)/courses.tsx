...
const LEVEL_COLORS: Record<string, { bg: string; text: string; darkBg: string; darkText: string }> = {
  '17-12-kyu': { bg: '#fef3c7', text: '#92400e', darkBg: '#451a03', darkText: '#fbbf24' },
  '11-6-kyu':  { bg: '#dbeafe', text: '#1e40af', darkBg: '#1e3a5f', darkText: '#93c5fd' },
  '5kyu-1dan': { bg: '#f3e8ff', text: '#6b21a8', darkBg: '#3b0764', darkText: '#d8b4fe' },
};
...
function CourseCard({ item, index, onPress, cardWidth }: {
  ...
  const lc = LEVEL_COLORS[lk] ?? { bg: '#f3f4f6', text: '#374151', darkBg: '#1e293b', darkText: '#94a3b8' };
  return (
    <Pressable onPress={onPress}
      style={{ width: cardWidth, marginBottom: 16 }}
      className="bg-white dark:bg-dark-card rounded-2xl overflow-hidden border border-gray-100 dark:border-dark-border active:opacity-90">
      ...
      <View className="p-4">
        <Text className="text-xs text-gray-400 dark:text-slate-500 uppercase tracking-wide mb-1">Agora Go Akademisi</Text>
        <Text className="text-base font-bold text-gray-900 dark:text-slate-100 leading-snug" numberOfLines={2}>{item.title}</Text>
        {item.summary ? (
          <Text className="text-sm text-gray-500 dark:text-slate-400 mt-1.5 leading-relaxed" numberOfLines={2}>{item.summary}</Text>
        ) : item.description ? (
          <Text className="text-sm text-gray-500 dark:text-slate-400 mt-1.5 leading-relaxed" numberOfLines={2}>{item.description}</Text>
        ) : null}
        <View className="flex-row items-center justify-between mt-3 pt-3 border-t border-gray-50 dark:border-dark-border">
          {formatDuration(item.duration_minutes) ? (
            <View className="flex-row items-center gap-1">
              <Ionicons name="time-outline" size={12} color="#9ca3af" />
              <Text className="text-xs text-gray-400 dark:text-slate-500">{formatDuration(item.duration_minutes)}</Text>
            </View>
          ) : <View />}
          <View className="w-8 h-8 rounded-full bg-blue-600 dark:bg-accent-blue items-center justify-center">
...
export default function CoursesScreen() {
  ...
  if (loading) {
    return (
      <View className="flex-1 bg-gray-50 dark:bg-dark-bg items-center justify-center" style={{ paddingTop: insets.top }}>
        <ActivityIndicator size="large" color="#1d4ed8" />
        <Text className="mt-3 text-gray-400 dark:text-slate-500 text-sm">Kurslar yükleniyor…</Text>
      </View>
    );
  }
  ...
  return (
    <View className="flex-1 bg-gray-50 dark:bg-dark-bg" style={{ paddingTop: insets.top }}>
      <View className="px-4 pt-6 pb-4">
        <Text className="text-2xl font-bold text-gray-900 dark:text-slate-100">Kurslar</Text>
        <Text className="mt-1 text-gray-500 dark:text-slate-400 text-sm">Temel Taşlar · Gelişim · Aydınlanma</Text>
      </View>

      {errorMsg ? (
        <View className="px-4 py-6 items-center">
          <Ionicons name="cloud-offline-outline" size={40} color="#d1d5db" />
          <Text className="text-gray-500 dark:text-slate-400 mt-2 text-sm text-center">{errorMsg}</Text>
        </View>
      ) : courses.length === 0 ? (
        <View className="px-4 py-12 items-center">
          <Ionicons name="school-outline" size={48} color="#d1d5db" />
          <Text className="text-gray-400 dark:text-slate-500 mt-3">Henüz kurs bulunamadı.</Text>
        </View>
      ) : (
        <FlatList
          data={courses}
